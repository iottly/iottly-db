...
mongod --fork --logpath /var/log/mongodb.log --dbpath /data/db/ \
	...
	&& for f in $INIT_FILES; do mongo --eval "
		var bridgeEncPwd='${MQTT_BRIDGE_ENC_PWD}';
		var bridgeUser='${MQTT_BRIDGE_USERNAME}'; 
		var commandTopic='${MQTT_COMMAND_TOPIC}';
		var posthttpUser='${MQTT_POSTHTTP_USERNAME}'; 
		var posthttpTopic='${MQTT_POSTHTTP_TOPIC}'; 
		" $f; echo $f; done \
	&& mongod --shutdown \
